...
var core = {
  ...
  init: function(){
    ...
    containers.ultrahard = $('#ultrabtn');
    containers.ultraeasy = $('#ueasybtn');
    containers.switchscreens = $('#switchscreens');
    containers.reset = $('#reset');
    
    var difarr = { 18:containers.ultraeasy,12: containers.easy, 9: containers.medium, 6: containers.insane, 3: containers.ultrahard};
  ...
  reset: function(){  
    ...
    containers.status.html('開始！');
    containers.time.html('0');
    containers.msg.html('四角をクリックしてつぶしてね');
...
var action = {
  ...
  click: function(e){  
    ...
    let chk = document.getElementById('flagchk');
    ...
    if(!chk.checked && globals.flagMap[x][y] !== 1 && defaults.difficulty !== 0){
      ...
      if(globals.firstClick === true){
        ...
        // Set number of mines
        containers.mines.html('あなたは ' + globals.totalMines + '個破壊しました');
      ...
      action.index(x, y);
  	...
  	}else if(chk.checked && util.is('revealed', x, y)){
		...
		}else if(chk.checked && l < 0 && globals.firstClick !== true){
  ...
  flag: function(flag, x, y){
    ...
    containers.mines.html('あと' + (globals.totalMines - globals.totalFlags) + '個の地雷が残っています');
    containers.flags.html('あなたは ' + globals.totalFlags + '個の旗を設置しました');
  ...
  revealMines: function(mine){
    ...
    // Set game over status
    globals.gameover = true;
    containers.status.html('あなたの負け');
    containers.msg.html('「リセット」をクリックしてもう一度遊ぶことができます');
  ...
  display: function(){
    ...
    if(typeof Storage !== 'undefined'){
      ...
      if(typeof localStorage.scores !== 'undefined'){
        ...
        containers.scoreboard.html('<tr><th>名前</th><th>個数</th><th>時間</th></tr>');
        ...
        containers.scoreboard.html('<tr><td>まだあなたはクリアしていません</td></tr>');
      }
    
    }else{
      containers.scoreboard.html('<tr><td>不明なエラーが発生しました</td></tr>');       
    } 
